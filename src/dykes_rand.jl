...
Ly::Float64 = 25000; # y size of area, m %20000
Lx_Ly       = Lx/Ly;
narrow_fact = 0.6;
dike_x_W    = 5000; #m 
...
if length(dikemxs)<=1;dikemxs = [0];end
		if length(dikemys)<=1;dikemys = [0];end
    dikemx,dikemy  = meshgrid(dikemxs,dikemys);
